...
app.post('/habits/:id/toggle', async (req, res) => {
    ...
    try {
        ...
        if (existsIndex > -1) {
            ...
            xpChange = -15; // Penalty for un-checking (Must match reward to prevent farming)
        } else {
        ...
        habit.currentStreak = habit.completedDates.length; // Simplified logic, real prod needs better calc
        ...
        if (user) {
            ...
            if (user.hellWeek?.isActive) {
                // If gaining XP (15), add bonus (20) = 35
                // If losing XP (-15), subtract bonus (-20) = -35
...
app.post('/user/:userId/hell-week', async (req, res) => {
    ...
    try {
        ...
        if (action === 'start') {
            ...
            user.platformXp = Math.max(0, user.platformXp - 500); // Big penalty
